...
public class MixinItemRenderer
{
    ...
    public float updateEquippedItem(final float n, final float n2, final float n3) {
        return MathHelper.clamp(Client.hNxW.qHPF((Class<LowShield>)PLVS.class).IuiN() ? (Client.hNxW.qHPF((Class<LowShield>)PLVS.class).height.OnGi().floatValue() - this.equippedProgressOffHand) : n, n2, n3);
    }
    
    @Inject(method = { "renderFireInFirstPerson" }, at = { @At("HEAD") }, cancellable = true)
    public void renderFireInFirstPerson(final CallbackInfo callbackInfo) {
        if (Client.hNxW.qHPF((Class<NoRender>)myLi.class).IuiN() && Client.hNxW.qHPF((Class<NoRender>)myLi.class).fire.OnGi()) {
            callbackInfo.cancel();
        }
    }
}
